...
#         harmonic_amplitude_sum    * component_harmonic_energy_ratio
#       + inharmonic_amplitude_sum  * component_inharmonic_energy_ratio
#       + subbass_amplitude_sum     * component_subbass_energy_ratio
...
def extract_density_components_from_per_note_workbook(
    ...
    *,
    ...
    result: Dict[str, Any] = {
        ...
        "harmonic_amplitude_source": "",
        # Stage 2 weighted note-density inputs. ``*_amplitude_sum`` sums
        # ``Amplitude_raw`` (with a single ``Amplitude`` fallback) over
        # the inharmonic / sub-bass spectrum sheets. Power_raw is NEVER
        # read on the default path. The weighted-density metric is:
        #
        #     density_weighted_sum =
        #         harmonic_amplitude_sum    * component_harmonic_energy_ratio
        #       + inharmonic_amplitude_sum  * component_inharmonic_energy_ratio
    ...
    if weight_function is not None:
        # Honour the operator's explicit weight_function: replace
        # D_H/D_I/D_S (and counts/sources) with the canonical values.
    ...
    # ------------------------------------------------------------------
    # Stage 2 weighted note-density metric (amplitude-basis components,
    # energy-ratio weights). Independent of Power_raw, legacy batch_* /
    # GUI weights, or workbook-max normalisation.
    # ------------------------------------------------------------------
    ...
    if can_compute_weighted_density:
        h_amp = result.get("harmonic_amplitude_sum")
        i_amp = result.get("inharmonic_amplitude_sum")
        s_amp = result.get("subbass_amplitude_sum")
        ...
        h_f = float(h_amp) if h_amp is not None and np.isfinite(float(h_amp)) else float("nan")
        i_f = float(i_amp) if i_amp is not None and np.isfinite(float(i_amp)) else float("nan")
        s_f = float(s_amp) if s_amp is not None and np.isfinite(float(s_amp)) else float("nan")
        weighted_h = h_f * wH_f
        weighted_i = i_f * wI_f
        weighted_s = s_f * wS_f
    ...
    *,
    ...
    for fpath, note, folder in files_list:
        ...
        row = {
            ...
            # === STAGE 2 LEGACY WEIGHTED NOTE-DENSITY (pre-per-band-log) ==
            # density_weighted_sum  = (raw amplitude × weight) summed; uses
            #   ``A_raw`` directly (no per-band log). Predates the canonical
            #   correction (density_metric_raw above), kept for back-compat.
            # density_log_weighted  = log10(1 + density_weighted_sum); this
            #   is "log on the outside" rather than the canonical
            #   "log per band, then weighted sum" \u2014 the two values
            #   differ for non-linear spectra and the canonical one
            #   above (density_metric_raw) is authoritative.
