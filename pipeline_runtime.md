...
| Stage 1B Per-note analysis | `proc_audio.AudioProcessor` + core acoustic/density modules (policy v2: spacing cap, exclusive slot assignment, f0/B refit, body-stop count cut, validated-partial gating, noise-gated mass) | per-note `spectral_analysis.xlsx` |
...
| `pipeline_orchestrator_integrated.py` | `RobustOrchestrator.run_stage1_per_note_analysis`, `run_stage2_compilation`, `run_complete_pipeline` | Three-stage orchestration and handoff to Stage 1/2/3 engines | `compiled_df = compile_density_metrics_with_pca(...)`; `run_research_workbook_export(...)` | N/A (pipeline control) |
...
| `proc_audio.py` | `AudioProcessor` methods (peak extraction, f0 refine, policy-v2 match, exclusive assignment, per-note export) | Per-note spectral analysis; spacing-capped match; one peak per slot; first-pass B as second-pass stretch | `tol_hz = max(bin, min(n*f0*tau/1200, beta*f0))` | \(\mathrm{tol}_{hz}(n)=\max(\Delta f_{\mathrm{bin}},\min(n f_0\tau_n/1200,\beta f_0))\) |
