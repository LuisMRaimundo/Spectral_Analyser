...
| Stage 1B Per-note analysis | `proc_audio.AudioProcessor` + core acoustic/density modules (policy v2: spacing cap, f0/B refit, body-stop count cut, noise-gated mass) | per-note `spectral_analysis.xlsx` |
...
| `proc_audio.py` | `AudioProcessor` methods (peak extraction, f0 refine, policy-v2 match, per-note export) | Per-note spectral analysis; spacing-capped match; first-pass B as second-pass stretch | `tol_hz = max(bin, min(n*f0*tau/1200, beta*f0))` | \(\mathrm{tol}_{hz}(n)=\max(\Delta f_{\mathrm{bin}},\min(n f_0\tau_n/1200,\beta f_0))\) |
