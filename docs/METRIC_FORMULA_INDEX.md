...
| F-057 | $r_k=E_k/\sum_j E_j$ (derived); $\mathrm{ACD}=\sum_k r_k D_{2,k}$ after ERB merge | Auditory Component Density (how many) | `tools.spectral_density_hill.compute_note_density` | `ACD_score` |
| F-058 | $\lambda=\sum_k E_k/\mathrm{ACD}$ | Magnitude per effective component (how large) | `tools.spectral_density_hill.compute_note_density` | `ACD_magnitude_per_component` |
...
| F-060 | Merge if $f_\mathrm{next}-f_\mathrm{centroid}\le \varepsilon\,\mathrm{ERB}(f_\mathrm{centroid})$; $\mathrm{ERB}(f)=0.108f+24.7$ | ERB peak merge; Tier B excitation scaffold | `tools.spectral_density_hill.merge_peaks_within_erb` | `ACD_count_merged_*`, `ACD_erb_fraction` |
