...
| F-008 | v2: WLS $f_n^2=an^2+cn^4$, $w_n\propto 1/f_n^2$, $B=c/a$ signed; n=1 excluded from $(a,c)$; global monotone assignment. $\|t\|\ge 2$ is a heuristic significance screen; residuals are dominated by systematic peak-frequency estimation error, so no formal coverage is claimed. Detection floor: under realistic peak-frequency jitter (~5 cents), $|B|\lesssim 2\times 10^{-5}$ — including negative stretch — is reported as not_significant rather than estimated; the practical estimation floor is $\approx 10^{-4}$ with relative accuracy of order a factor 1.5–2. B is a stretch descriptor at this accuracy, not a precision physical measurement. String-family only (Fletcher 1962). | Inharmonicity B / phenomenological stretch | `inharmonicity_model.fit_inharmonicity_coefficient` | `inharmonicity_coefficient_B`, `spectral_stretch_coefficient` |
...
| F-037 | $x=\frac{|f_i-f_j|}{0.25\,\mathrm{CB}(f_\mathrm{lo})}$; default $\mathrm{CB}=$ Zwicker $25+75(1+1.4(f/1000)^2)^{0.69}$. Drop pairs with $\max(f_i,f_j)>15500\,\mathrm{Hz}$. Alt. `erb`: $0.25(0.108f+24.7)$. $g(x)=xe^{1-x}$ | Parncutt / Plomp–Levelt pairwise roughness. Provenance-consistent default. Bark-scale ceiling on Zwicker CB. | `mir_descriptors.roughness_parncutt_kernel` | `roughness_parncutt_kernel` |
...
| F-045 | $N_{eff}=(\sum_n A_n^2)^2/\sum_n A_n^4$ | Energy-distribution density ("fatness"): effective number of harmonic partials carrying energy (register-robust, timbre-discriminating) | `compile_metrics._energy_distribution_density` | `harmonic_effective_partial_count` |
| F-047 | $N_{eff}^{HIS}=(\sum_i A_i^2)^2/\sum_i A_i^4$ over harmonic ∪ inharmonic ∪ sub-bass components | Unified single-scalar note density across all three bands; per-note basis for chord/aggregate density (pooled participation ratio) | `compile_metrics._energy_distribution_density` | `note_effective_component_density` (+ `*_ci_low/ci_high/rel_uncertainty`; algebra unchanged) |
| F-048 | $\text{EWSD}_k=r_k D_k(N_{\mathrm{eff},k}/N_k)$; $\text{EWSD}=\sum_k \text{EWSD}_k$ | Strict EWSD (Stage 3 anti-concentration density). **d10 note:** `original_sum_metric` already multiplies $\sum\log(1+A)$ by energy-share $N_{\mathrm{eff}}/N$; the F-048 penalty then applies a second $N_{\mathrm{eff}}/N$ on $\log(1+A)$ shares. Algebra frozen; thesis-safe membership is an open item. | `tools/ewsd_core.compute_ewsd` | `EWSD_score_total` |
| F-049 | $\text{EWSD}^{bal}_k=r_k D_k(N_{\mathrm{eff},k}/N_k)^{\alpha}$, $\alpha=0.5$ | Acoustic-balanced EWSD companion. **diagnostic only; level-dependent; not for cross-note comparison.** Computation unchanged. | `tools/ewsd_core.add_acoustic_alignment_columns` | `EWSD_score_acoustic_balanced` |
| F-050 | Percentile and BCa intervals from with-replacement resampling of H/I/S partials. **Sensitivity analysis**, not a coverage-bearing CI: partials are a deterministic structure, not an i.i.d. superpopulation draw. Label: `partial_multiset_sensitivity`. Not sampled: peak-picking threshold, FFT length/tier, window, f0 error, H/I/S assignment. | EWSD partial-multiset sensitivity | `tools.ewsd_uncertainty.bootstrap_ewsd_from_compartments` | `EWSD_score_*_ci_*`, `*_ci_*_bca`, `ewsd_uncertainty_sources` |
| F-057 | $r_k=E_k/\sum_j E_j$ (derived); $\mathrm{ACD}=\sum_k r_k D_{1,k}$ after ERB merge. Previous D2-based value retained as `ACD_score_D2_dominance`. | Auditory Component Density (how many) | `tools.spectral_density_hill.compute_note_density` | `ACD_score` |
| F-058 | $\lambda=\sum_k E_k/\mathrm{ACD}$ (D1-based; $E=\mathrm{ACD}\cdot\lambda$) | Magnitude per effective component (how large) | `tools.spectral_density_hill.compute_note_density` | `ACD_magnitude_per_component` |
...
| F-061 | v2: $\mathrm{count}_k=(D_{0,k}D_{1,k})^{0.5}$; $\mathrm{count}=\sum_k r_k\,\mathrm{count}_k$; $\lambda=E/\mathrm{count}$; $\mathrm{spectral\_mass}=\mathrm{count}\,\lambda^{0.15}$. Cross-compartment contributions bounded by energy share; within-compartment counting blends presence (D0) and share-weighted (D1). NaN if any required input is NaN or `ACD_status` $\ne$ `ok`. External explanatory documents dated before v2 describe the v1 pooled-D0 count; the worked example (F#4 = 21.0413) is a v1 value. v2 worked example (synthetic split): $D0_H=24$, $D1_H=1.10$, $r_H=0.96$, $D0_I=40$, $D1_I=2.5$, $r_I=0.03$, $D0_S=15$, $D1_S=1.8$, $r_S=0.01$, $E=10589.223$ → `spectral_mass` = 16.5305. | Spectral mass (how much is sounding). Presence-first count × bounded level. **Supersedes any informal use of `EWSD_score_acoustic_balanced` as a mass/fullness proxy.** | `tools.spectral_mass.add_spectral_mass_column` | `spectral_mass`, `spectral_mass_count` |
...
| F-069 | Stage-1 `{harmonic,inharmonic,subbass}_energy_ratio`: H and S overwritten from acoustic-core PSD shares of $h+r+s$; I is discrete inharmonic peak-PSD share of $H+I+S$ | Plain energy-ratio triple | `proc_audio` + `acoustic_density_core` | `harmonic_energy_ratio`, `inharmonic_energy_ratio`, `subbass_energy_ratio` |
| F-070 | `core_*` export of `component_*` power shares; residual = inharmonic component share; denominator $H_n+I_n+S_n$ | Core energy-ratio triple | `proc_audio._build_main_metrics_export_row` | `core_harmonic_energy_ratio`, `core_residual_energy_ratio`, `core_subbass_energy_ratio` |
