...
| F-037 | $x=\frac{|f_i-f_j|}{0.25\,\mathrm{CB}(f_\mathrm{lo})}$; default $\mathrm{CB}=$ Zwicker $25+75(1+1.4(f/1000)^2)^{0.69}$. Alt. `erb`: $0.25(0.108f+24.7)$. $g(x)=xe^{1-x}$ | Parncutt / Plomp–Levelt pairwise roughness. Proposed default, not author-validated. | `mir_descriptors.roughness_parncutt_kernel` | `roughness_parncutt_kernel` |
