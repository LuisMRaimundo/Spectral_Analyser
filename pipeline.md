...
| `proc_audio.py` | `AudioProcessor` methods (peak extraction, f0 refine, policy-v2 match, per-note exports) | End-to-end per-note analysis; spacing-capped `tol_hz(n)`; first-pass B is second-pass stretch; writes spectral/workbook outputs | `tol_hz = max(bin, min(n*f0*tau/1200, beta*f0))` | `\\mathrm{tol}_{hz}(n)=\\max(\\Delta f_{\\mathrm{bin}},\\min(n f_0\\tau_n/1200,\\beta f_0))` | P1/P4/P5: per-note runtime execution |
