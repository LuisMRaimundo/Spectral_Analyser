...
This document covers operational scripts in `Spectral_Analyser` (runtime, orchestration, validation, export, GUI worker, and support modules), excluding test files.
...
| P1 Per-note spectral extraction | Peaks, harmonic/non-harmonic split, f0 provenance (H1–H8 refit when `|Δ| > 15` c), spacing-capped match (β = 0.30), exclusive peak-to-slot assignment, body-stop validation cut, validated-partial gating (F-012 / pies / Sethares), noise-gated core integrals, density descriptors | `proc_audio.py`, `validated_partials.py`, `acoustic_density_core.py`, `density.py`, `harmonic_alignment.py`, `harmonic_peak_validation.py`, `harmonic_validation.py`, `spectral_leakage_guards.py`, `peak_component_counts.py` |
...
|---|---|---|---|---|---|
| `acoustic_density_core.py` | `canonical_f0_triplet`, `_expected_harmonic_orders`, `_expected_residual_bin_count`, `compute_acoustic_density_descriptors`, `noise_gated_power` | Builds per-note acoustic descriptors, f0 epistemic state, H/I/S observation weights, diagnostics; core peak-power integrals subtract the floor when the gate is on | `data_ratio = component_strength / sum(component_strength)` | `w_k = \\frac{s_k}{\\sum_j s_j}` | P1/P4/P7: main per-note descriptor engine |
...
| `mir_descriptors.py` | `compute_mir_descriptors_from_spectrum`, `_roughness_parncutt_kernel`, `_erb_rate_hz` | Computes spectral moments, tristimulus, flatness, rolloff, Parncutt/Plomp–Levelt roughness, ERB-weighted descriptors | `centroid = sum(f*p)` | `\\mu_f=\\sum_i f_i p_i` | P5: descriptor extension |
...
| `proc_audio.py` | `AudioProcessor` methods (peak extraction, f0 refine, policy-v2 match, exclusive assignment, per-note exports) | End-to-end per-note analysis; spacing-capped `tol_hz(n)`; one peak per slot; first-pass B is second-pass stretch; writes spectral/workbook outputs | `tol_hz = max(bin, min(n*f0*tau/1200, beta*f0))` | `\\mathrm{tol}_{hz}(n)=\\max(\\Delta f_{\\mathrm{bin}},\\min(n f_0\\tau_n/1200,\\beta f_0))` | P1/P4/P5: per-note runtime execution |
...
| `harmonic_peak_validation.py` | `cfar_peak_detection`, `refine_f0_from_low_order_peaks`, `apply_harmonic_body_stop`, `noise_gated_linear_mass`, `_classify_harmonic_candidate` | Per-order classification, H1–H8 f0/B refit, body-stop count cut, noise-gated mass (re-exported by `proc_audio`) | CFAR + saddle prominence + spacing-capped window; stop is validation/count only |
